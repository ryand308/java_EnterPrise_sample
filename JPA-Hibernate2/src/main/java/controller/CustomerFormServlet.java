...
public class CustomerFormServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if( id != null) {
			...
			response.sendRedirect("http://localhost:8080/testHibernate2/CustomerListServlet");
		}
		else if(service.addCustomer(country, firstName, lastName))
			response.sendRedirect("http://localhost:8080/testHibernate2/formSuccess.html");		
		else
			response.sendRedirect("http://localhost:8080/testHibernate2/formFailure.html");
